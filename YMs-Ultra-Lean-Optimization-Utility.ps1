...
$Sections["Network & Internet"] = @(
    ...
    [PSCustomObject]@{ Name="Disable SMBv1"; Cat="Network & Internet"; Ben=WARNING! "Improves security by disabling old SMB version."; Cmd={ Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters" -Name "SMB1" -Value 0 } },
...
$Sections["Privacy & Security"] = @(
    ...
    [PSCustomObject]@{ Name="Disable SmartScreen"; Cat="Privacy & Security"; Ben=REDUCES SECURITY! "Prevents Windows from scanning apps online."; Cmd={ Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer" -Name "SmartScreenEnabled" -Value "Off" } },
