...
$Global:AchievementsPath = "$env:USERPROFILE\YMsWinUtil_Achievements.json"
$Global:CloudSyncPath = "$env:USERPROFILE\YMsWinUtil_Cloud.json"
$Global:BenchmarkPath = "$env:USERPROFILE\YMsWinUtil_Benchmarks.json"
$Global:SchedulePath = "$env:USERPROFILE\YMsWinUtil_Schedules.json"
$Global:Progress = 0
$Global:UserPoints = 0
$Global:UserStreak = 0
...
# Enhanced Logging with System Health
function Write-Log {
    ...
    if ($Points -gt 0) {
        Add-Points -Points $Points -Reason $Message
    }
}
...
try {
    ...
    # ===============================================
# YMs Windows Control Center - ULTIMATE v10.1.0
# NEW: Winget Installer, Safe-Guard, & Power Toggles
# ===============================================

# [Previous variables and logic remain, adding the following new modules]

# 1. THE SAFE-GUARD: Automatic Restore Point
function Enable-BeastSafety {
    Write-Log "🛡️ Initializing Safe-Guard System Restore Point..." "SECURITY"
    try {
        Enable-ComputerRestore -Drive "C:\"
        Checkpoint-Computer -Description "Before YM Ultimate Optimization" -RestorePointType "MODIFY_SETTINGS"
        Write-Log "✅ Safety Net Created. You can rollback if things get too 'Beast Mode'." "SUCCESS"
    } catch {
        Write-Log "⚠️ Restore Point failed. Ensure WMI service is active." "ERROR"
    }
}

# 2. THE BEAST INSTALLER: Winget Integration
function Install-AppSuite {
    param([string[]]$AppIDs)
    Write-Log "🚀 Starting AI-Driven Bulk App Installation..." "AI"
    
    foreach ($ID in $AppIDs) {
        Write-Log "Checking for $ID..." "INFO"
        $Check = winget list --id $ID -e
        if ($Check -match $ID) {
            Write-Log "$ID is already installed. Skipping." "SUCCESS"
        } else {
            Write-Log "Installing $ID..." "GAMING"
            winget install --id $ID --silent --accept-package-agreements --accept-source-agreements
            Add-Points -Points 150 -Reason "Installed essential app: $ID"
        }
    }
}

# 3. PERFORMANCE TOGGLE: Ultimate Power Plan
function Set-BeastPowerPlan {
    Write-Log "⚡ Unleashing Ultimate Power Plan..." "GAMING"
    # GUID for Ultimate Performance (hidden in many Windows versions)
    $UltimateGUID = "e9a42b02-d5df-448d-aa00-03f14749eb61"
    
    $Check = powercfg /list
    if ($Check -match $UltimateGUID) {
        powercfg /setactive $UltimateGUID
    } else {
        powercfg /duplicatescheme $UltimateGUID
        powercfg /setactive $UltimateGUID
    }
    Write-Log "🔥 CPU Throttling Disabled. Hardware is now in Beast Mode." "SUCCESS"
}

# 4. ENHANCED MAIN EXECUTION FLOW
function Start-UltimateOptimization {
    # 1. Safety First
    Enable-BeastSafety
    
    # 2. Analyze
    $Analysis = Get-SystemAnalysis
    Write-Host "Current System Health: $($Analysis.HealthScore)%" -ForegroundColor Cyan
    
    # 3. Apply Core Tweaks
    Set-BeastPowerPlan
    
    # 4. Optional App Installation (Examples)
    $EssentialApps = @(
        "7zip.7zip", 
        "Google.Chrome", 
        "Discord.Discord", 
    ...
    # Install-AppSuite -AppIDs $EssentialApps
    
    Update-Streak
    Write-Log "✨ Optimization Cycle Complete!" "SUCCESS"
}

# Run the beast
# Start-UltimateOptimization
